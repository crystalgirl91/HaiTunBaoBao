...
angular.module("products.pay.failure", ["ng", "common","directives"])
.controller("productsPayFailureController", ["$scope", "$API","queryParams", function($scope, $API,queryParams){
	...
	$scope.repay = function(){
		$API.insureCommAuPolicyPay(ordAid).then(
			res =>{
				...
				let furl = baseurl + "/products_pay_failure.html?" + $.param({
					ordAid:ordAid,
